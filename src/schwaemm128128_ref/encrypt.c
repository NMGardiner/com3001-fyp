...
#include "sparkle_ref/sparkle_ref.h"
...
void s128128ref_Initialize(uint32_t *state, const uint8_t *key, const uint8_t *nonce)
{
...
void s128128ref_ProcessAssocData(uint32_t *state, const uint8_t *in, size_t inlen)
{
...
void s128128ref_ProcessPlainText(uint32_t *state, uint8_t *out, const uint8_t *in, \
                      size_t inlen)
{
...
void s128128ref_Finalize(uint32_t *state, const uint8_t *key)
{
...
void s128128ref_GenerateTag(uint32_t *state, uint8_t *tag)
{
...
int s128128ref_VerifyTag(uint32_t *state, const uint8_t *tag)
{
...
void s128128ref_ProcessCipherText(uint32_t *state, uint8_t *out, const uint8_t *in, \
                       size_t inlen)
{
  // check whether 'in' and 'out' can be casted to uint32_t pointer
  int aligned = (((size_t) in) | ((size_t) out)) % UI32_ALIGN_BYTES == 0;
...
int s128128ref_crypto_aead_encrypt(UChar *c, ULLInt *clen, const UChar *m, ULLInt mlen, \
  ...
  const UChar *k)
{
  ...
  s128128ref_Initialize(state, k, npub);
  if (adsize) s128128ref_ProcessAssocData(state, ad, adsize);
  if (msize) s128128ref_ProcessPlainText(state, c, m, msize);
  s128128ref_Finalize(state, k);
  s128128ref_GenerateTag(state, (c + msize));
  *clen = msize;
  *clen += TAG_BYTES;
...
int s128128ref_crypto_aead_decrypt(UChar *m, ULLInt *mlen, UChar *nsec, const UChar *c, \
  ...
  const UChar *k)
{
  ...
  s128128ref_Initialize(state, k, npub);
  if (adsize) s128128ref_ProcessAssocData(state, ad, adsize);
  if (csize) s128128ref_ProcessCipherText(state, m, c, csize);
  s128128ref_Finalize(state, k);
  retval = s128128ref_VerifyTag(state, (c + csize));
  *mlen = csize;
