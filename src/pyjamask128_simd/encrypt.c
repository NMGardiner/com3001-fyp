...
#include "api.h"
#define KEYBYTES   PJ128SIMD_CRYPTO_KEYBYTES
...
static int pj128simd_ocb_crypt(unsigned char *out, unsigned char *k, unsigned char *n,
                     unsigned char *a, unsigned abytes,
...
void pj128simd_ocb_encrypt(unsigned char *c, unsigned char *k, unsigned char *n,
                 unsigned char *a, unsigned abytes,
                 unsigned char *p, unsigned pbytes) {
    pj128simd_ocb_crypt(c, k, n, a, abytes, p, pbytes, OCB_ENCRYPT);
}

/* ------------------------------------------------------------------------- */

int pj128simd_ocb_decrypt(unsigned char *p, unsigned char *k, unsigned char *n,
                unsigned char *a, unsigned abytes,
                unsigned char *c, unsigned cbytes) {
    return pj128simd_ocb_crypt(p, k, n, a, abytes, c, cbytes, OCB_DECRYPT);
}
...
)
{
    ...
    *clen = mlen + TAGBYTES;
    pj128simd_ocb_crypt(c, (unsigned char *)k, (unsigned char *)npub, (unsigned char *)ad,
...
)
{
    (void) (nsec); // unused argument
    *mlen = clen - TAGBYTES;
    return pj128simd_ocb_crypt(m, (unsigned char *)k, (unsigned char *)npub,
