...
void e384simd_Initialize(uint32_t *state)
{
  int i;
...
void e384simd_ProcessMessage(uint32_t *state, const UChar *in, size_t inlen)
{
...
void e384simd_Finalize(uint32_t *state, UChar *out)
{
  size_t outlen;
...
int e384simd_crypto_hash(UChar *out, const UChar *in, ULLInt inlen)
{
  ...
  e384simd_Initialize(state);
  e384simd_ProcessMessage(state, in, insize);
  e384simd_Finalize(state, out);
