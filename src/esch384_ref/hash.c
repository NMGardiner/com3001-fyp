...
void e384ref_Initialize(uint32_t *state)
{
  int i;
...
void e384ref_ProcessMessage(uint32_t *state, const UChar *in, size_t inlen)
{
...
void e384ref_Finalize(uint32_t *state, UChar *out)
{
  size_t outlen;
...
int e384ref_crypto_hash(UChar *out, const UChar *in, ULLInt inlen)
{
  ...
  e384ref_Initialize(state);
  e384ref_ProcessMessage(state, in, insize);
  e384ref_Finalize(state, out);
