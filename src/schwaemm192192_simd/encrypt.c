...
void s192192simd_Initialize(uint32_t *state, const uint8_t *key, const uint8_t *nonce)
{
...
void s192192simd_ProcessAssocData(uint32_t *state, const uint8_t *in, size_t inlen)
{
...
void s192192simd_ProcessPlainText(uint32_t *state, uint8_t *out, const uint8_t *in, \
                      size_t inlen)
{
...
void s192192simd_Finalize(uint32_t *state, const uint8_t *key)
{
...
void s192192simd_GenerateTag(uint32_t *state, uint8_t *tag)
{
...
int s192192simd_VerifyTag(uint32_t *state, const uint8_t *tag)
{
...
void s192192simd_ProcessCipherText(uint32_t *state, uint8_t *out, const uint8_t *in, \
                       size_t inlen)
{
  // check whether 'in' and 'out' can be casted to uint32_t pointer
  int aligned = (((size_t) in) | ((size_t) out)) % UI32_ALIGN_BYTES == 0;
  ...
  const UChar *k)
{
  ...
  s192192simd_Initialize(state, k, npub);
  if (adsize)s192192simd_ProcessAssocData(state, ad, adsize);
  if (msize) s192192simd_ProcessPlainText(state, c, m, msize);
  s192192simd_Finalize(state, k);
  s192192simd_GenerateTag(state, (c + msize));
  *clen = msize;
  *clen += TAG_BYTES;
  ...
  const UChar *k)
{
  ...
  s192192simd_Initialize(state, k, npub);
  if (adsize) s192192simd_ProcessAssocData(state, ad, adsize);
  if (csize) s192192simd_ProcessCipherText(state, m, c, csize);
  s192192simd_Finalize(state, k);
  retval = s192192simd_VerifyTag(state, (c + csize));
  *mlen = csize;
