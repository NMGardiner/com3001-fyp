...
void e256simd_Initialize(uint32_t *state)
{
  int i;
...
void e256simd_ProcessMessage(uint32_t *state, const UChar *in, size_t inlen)
{
...
void e256simd_Finalize(uint32_t *state, UChar *out)
{
  size_t outlen;
...
int e256simd_crypto_hash(UChar *out, const UChar *in, ULLInt inlen)
{
  ...
  e256simd_Initialize(state);
  e256simd_ProcessMessage(state, in, insize);
  e256simd_Finalize(state, out);
