...
void linear_layer_simd(SparkleState *state, int brans)
{
  int i, b = brans/2;
...
void sparkle_simd(SparkleState *state, int brans, int steps)
{
  ...
  for(i = 0; i < steps; i++) {
    ...
    // Linear layer
    linear_layer_simd(state, brans);
  }
}


void linear_layer_inv_simd(SparkleState *state, int brans)
{
  int i, b = brans/2;
  uint32_t *x = state->x, *y = state->y;
  uint32_t tmp;
...
void sparkle_inv_simd(SparkleState *state, int brans, int steps)
{
  int i, j;  // Step and branch counter
  ...
  for(i = steps - 1; i >= 0; i--) {
    // Linear layer
    linear_layer_inv_simd(state, brans);
    // ARXbox layer
    for(j = 0; j < brans; j ++)
...
void clear_state_simd(SparkleState *state, int brans)
{
  int i;
...
void print_state_simd(const SparkleState *state, int brans)
{
