...
#include "sparkle_simd/sparkle_simd.h"
...
void s256128simd_Initialize(uint32_t *state, const uint8_t *key, const uint8_t *nonce)
{
  ...
  sparkle_simd(state, STATE_BRANS, SPARKLE_STEPS_BIG);
}
...
void s256128simd_ProcessAssocData(uint32_t *state, const uint8_t *in, size_t inlen)
{
  ...
  while (inlen > RATE_BYTES) {
    ...
    sparkle_simd(state, STATE_BRANS, SPARKLE_STEPS_SLIM);
    inlen -= RATE_BYTES;
    in += RATE_BYTES;
  }
...
void s256128simd_ProcessPlainText(uint32_t *state, uint8_t *out, const uint8_t *in, \
                      size_t inlen)
{
...
void s256128simd_Finalize(uint32_t *state, const uint8_t *key)
{
...
void s256128simd_GenerateTag(uint32_t *state, uint8_t *tag)
{
...
int s256128simd_VerifyTag(uint32_t *state, const uint8_t *tag)
{
...
void s256128simd_ProcessCipherText(uint32_t *state, uint8_t *out, const uint8_t *in, \
                       size_t inlen)
{
  // check whether 'in' and 'out' can be casted to uint32_t pointer
  int aligned = (((size_t) in) | ((size_t) out)) % UI32_ALIGN_BYTES == 0;
  ...
  const UChar *k)
{
  ...
  s256128simd_Initialize(state, k, npub);
  if (adsize) s256128simd_ProcessAssocData(state, ad, adsize);
  if (msize) s256128simd_ProcessPlainText(state, c, m, msize);
  s256128simd_Finalize(state, k);
  s256128simd_GenerateTag(state, (c + msize));
  *clen = msize;
  *clen += TAG_BYTES;
  ...
  const UChar *k)
{
  ...
  s256128simd_Initialize(state, k, npub);
  if (adsize) s256128simd_ProcessAssocData(state, ad, adsize);
  if (csize) s256128simd_ProcessCipherText(state, m, c, csize);
  s256128simd_Finalize(state, k);
  retval = s256128simd_VerifyTag(state, (c + csize));
  *mlen = csize;
