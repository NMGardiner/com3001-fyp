...
void e256ref_Initialize(uint32_t *state)
{
  int i;
...
void e256ref_ProcessMessage(uint32_t *state, const UChar *in, size_t inlen)
{
...
void e256ref_Finalize(uint32_t *state, UChar *out)
{
  size_t outlen;
...
int e256ref_crypto_hash(UChar *out, const UChar *in, ULLInt inlen)
{
  ...
  e256ref_Initialize(state);
  e256ref_ProcessMessage(state, in, insize);
  e256ref_Finalize(state, out);
