...
#include "esch_cfg.h"
#include "sparkle-simd/sparkle_simd.h"
...
void Initialize_simd(SparkleState *state)
{
  int i;
...
void ProcessMessage_simd(SparkleState *state, const UChar *in, size_t inlen)
{
  // Main Hashing Loop
...
void Finalize_simd(SparkleState *state, UChar *out)
{
...
int crypto_hash_simd(UChar *out, const UChar *in, ULLInt inlen)
{
  ...
  Initialize_simd(&state);
  ProcessMessage_simd(&state, in, insize);
  Finalize_simd(&state, out);
