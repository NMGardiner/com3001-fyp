...
#include "pyjamask128_ref/pyjamask.h"
...
//    AES_set_encrypt_key(k, KEYBYTES*8, &aes_key);
    memset(tmp, 0, 16);
    pj128ref_pyjamask_128_enc(tmp, k, lstar);
    /* L_$ = double(L_*) */
    ...
    for (i=1; i<=abytes/16; i++, a = a + 16) {
        ...
        pj128ref_pyjamask_128_enc(tmp, k, tmp);
...
static int pj128ref_ocb_crypt(unsigned char *out, unsigned char *k, unsigned char *n,
    ...
    nonce[15] &= 0xC0;
    pj128ref_pyjamask_128_enc(nonce, k, ktop);
    ...
    for (i=1; i<=inbytes/16; i++, in=in+16, out=out+16) {
        ...
        if (encrypting) {
            ...
            pj128ref_pyjamask_128_dec(tmp, k, tmp);
            xor_block(out, offset, tmp);
            /* Checksum_i = Checksum_{i-1} xor P_i */
    ...
    if (inbytes > 0) {
        ...
        pj128ref_pyjamask_128_enc(offset, k, pad);
    ...
    pj128ref_pyjamask_128_enc(tmp, k, tag);
