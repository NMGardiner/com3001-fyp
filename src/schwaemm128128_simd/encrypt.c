...
void s128128simd_Initialize(uint32_t *state, const uint8_t *key, const uint8_t *nonce)
{
...
void s128128simd_ProcessAssocData(uint32_t *state, const uint8_t *in, size_t inlen)
{
...
void s128128simd_ProcessPlainText(uint32_t *state, uint8_t *out, const uint8_t *in, \
                      size_t inlen)
{
...
void s128128simd_Finalize(uint32_t *state, const uint8_t *key)
{
...
void s128128simd_GenerateTag(uint32_t *state, uint8_t *tag)
{
...
int s128128simd_VerifyTag(uint32_t *state, const uint8_t *tag)
{
...
void s128128simd_ProcessCipherText(uint32_t *state, uint8_t *out, const uint8_t *in, \
                       size_t inlen)
{
  // check whether 'in' and 'out' can be casted to uint32_t pointer
  int aligned = (((size_t) in) | ((size_t) out)) % UI32_ALIGN_BYTES == 0;
  ...
  const UChar *k)
{
  ...
  s128128simd_Initialize(state, k, npub);
  if (adsize) s128128simd_ProcessAssocData(state, ad, adsize);
  if (msize) s128128simd_ProcessPlainText(state, c, m, msize);
  s128128simd_Finalize(state, k);
  s128128simd_GenerateTag(state, (c + msize));
  *clen = msize;
  *clen += TAG_BYTES;
  ...
  const UChar *k)
{
  ...
  s128128simd_Initialize(state, k, npub);
  if (adsize) s128128simd_ProcessAssocData(state, ad, adsize);
  if (csize) s128128simd_ProcessCipherText(state, m, c, csize);
  s128128simd_Finalize(state, k);
  retval = s128128simd_VerifyTag(state, (c + csize));
  *mlen = csize;
